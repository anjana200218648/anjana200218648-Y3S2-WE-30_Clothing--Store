...
# 🔹 Home Page Route (Fixes 404 Error)
@app.route('/')
def home():
    return "Welcome to the Sales Tracker API! Use POST /upload to upload data."

# 🔹 Function to Train ARIMA Model
def train_arima_model(data):
    data['Month'] = pd.date_range(start='1/1/2020', periods=len(data), freq='M')
    data.set_index('Month', inplace=True)
    model = ARIMA(data['Sales'], order=(5,1,0))  # Adjust p,d,q as needed
...
def upload_file():
    ...
    try:
        ...
        df_combined.to_csv(DB_FILE, index=False)  # Save updated data
        
        prediction = train_arima_model(df_combined)
        return jsonify({'sales_data': df_combined.to_dict(orient='records'), 'next_month_sales_prediction': prediction})
